...
#define panorama_cSNOWr 740.0
/*940 930 920 */

#define panorama_cSNOWg 730.0

#define panorama_cSNOWb 720.0
...
static void AddShadow(double dist, double alt)
{
   pDROPSHADOW s;
   if (newshadow && newshadow->dist>=dist) {
      /* trace goes back */
      if (newshadow->alt<alt) newshadow->alt = alt;
   }
   else {
      if (freeshadow) {
         s = freeshadow;
         freeshadow = freeshadow->next;
      }
      else osic_alloc((char * *) &s, sizeof(struct DROPSHADOW));
      if (s) {
         s->next = newshadow;
         newshadow = s;
         s->dist = dist;
                ...
                struct TREEHINT * th)
{
   ...
   { /* with */
      ...
      anonym->peakh = anonym->areapeak*(1.0f-noise*5.340576171875E-6f);
                /* random tree size */
...
static void raytrace(double x0, double y00, double z0,
                ...
                TREEHINTS treehints, char * istree, char * odd,
                 char shad)
{
   float tolw;
   float tol;
   ...
   for (;;) {
      ...
      if (h<10000.0) {
         ...
         *istree = 0;
         ...
         if ((double)sp<(double)treehints[0].missspace* *dist) {
            /* store lowest pass over earth/tree */
            treehints[0].missspace = (float)(X2C_DIVL((double)sp,
                *dist));
         }
         if (shad) AddShadow(*dist, h+(double)sp);
         if (sp>tolw) {
            /* over ground */
                ...
                float refrac)
{
   ...
   raytrace(x0, y00, z0, xn, yn, zn, maxdist, 0.0f, refrac, 0.0, &dd, &space,
                 &pos, th, &istree, &odd, 0);
   if (dd>(double)maxdist) return th[0].missspace*1.0417414457222E+2f;
   return 0.0f;
} /* end seesun() */
...
static void sundisk(float wx, float wy, float sunazi,
                ...
                float alt, float refrac, struct COL * c)
{
   float lens;
   float ed;
   float s;
   ...
   if (dist==0.0f) {
      ...
      hc.g = hc.g*0.5f+s;
      hc.b = hc.b*0.5f+s;
      colblend((float)((0.5-sqrt((double)d)*3.0)*(double)
   ...
   if (dist==0.0f) {
      ...
      */
      s = maxdist*4.2441318159052E-5f; /* no blue sky on short sight */
      if (s>6.3661977238578E-1f) s = 6.3661977238578E-1f;
      if (wy<0.0f) wy = 0.0f;
      ed = 1.0f-wy*s;
      s = 3.0f+(float)fabs(x)+(float)fabs(sunele);
      if (s>6.0f) s = 6.0f;
      if (ed<=0.0f) ed = 0.0f;
   ...
   if (lenseffect==1U) {
      /* direct sun */
      if (d<=2.3036692370053E-5f) {
         /* view into sun */
         lens = 5000.0f;
      }
      else {
         lens = 0.3f+inv((float)fabs(sin((double)(atang2(x,
                y)*7.0f))));
      ...
      if (lens>0.1f) {
         s = seesun(pos, (double)alt, sunazi,
                ...
                float sunlum)
{
   ...
   if (xsun>0.0f) {
      /* surface sees sun */
      c->r = c->r+740.0f*xsun*sun.r;
      c->g = c->g+730.0f*xsun*sun.g;
      c->b = c->b+720.0f*xsun*sun.b;
   }
} /* end snowcol() */


static void botaniccol(struct COL * c, const struct COL suncol,
                float suncos, float mainsun, float diffuslum)
{
   float x;
   /*  x:=(xsun+2.5)*(sun.r+sun.g+sun.b)*0.33333333; */
   if (suncos<(-0.3f)) suncos = (-0.3f);
   x = (suncos+5.0f)*diffuslum;
   c->r = 0.05f*x;
   c->g = 0.055f*x;
   c->b = 0.09f*x;
   if (suncos>0.0f) {
      suncos = suncos*mainsun;
...
static void treecol(struct POSITIONL pos, float rayang,
                struct TREEHINT * th, const struct ROTVEC sunv,
                float pixsize, float sunlum, float dense,
                float snowdense, float mainsun, uint8_t hitwood,
                const struct COL suncol, float * hitin, struct COL * c)
{
   float pixshh;
   float pixsh;
   float dens;
   ...
   *hitin = (float)fabs(frac((double)((rayang+ang)
   ...
   trestruc = (float)fabs(rnd)*pixs+plus(0.75f-pixs);
   slant = 1.0f;
   if (th->peakh>0.0f) {
   ...
   if (pixs>0.0f) {
      ...
      sun = pixs*sun+(1.0f-pixs)*mainsun;
   ...
   if (th->isstam) {
      ...
      h = sh*(1.0f-one((0.8f-X2C_DIVR(th->hith,
                th->areapeak))*2.5f)*1.2f*dens*pixsh);
                /* use relative hight dark in wood */
      snowdense = one(snowdense*1.6f-pixsh*(2.2f-(float)
                fabs(qrand((double)((float)
                (th->nx*13L+th->ny*6L&511L)+ang*3.0f),
                (double)(th->og*4.0f)))));
      snowdense = snowdense*pixsh+snowdense*qrand((double)(float)
                th->nx, (double)(float)th->ny)*(0.1f-pixsh)*pixshh+snowdense*(0.35f-pixshh*0.35f);
      h = h+pixsh*one((X2C_DIVR(th->hith,th->regionpeak)-0.8f)*4.0f)*0.6f;
   ...
   c->b = c->b*bi;
   if (snowdense!=0.0f) {
      snowcol(&s, suncol, (sun*pixsh+mainsun*0.5f)*(1.0f-pixsh*0.5f),
                sunlum);
         ...
         if (wy>=(-1.5690509974981) && wy<1.5690509974981) {
            ...
            if (!heaven) {
               ...
               if (mirrord) {
                ...
                treehints, &istree, &odd, dropshadows);
                  ddm = dd;
            ...
            shadowcolour(1.0f+panpar.sunele*15.0f, suncol, &suncolair);
            heavenc = heaven;
            if (!heaven) {
               normvec(*pos, pmeta, &slant, &oldh, &nvec, &attr, &attw,
                &attu);
               /*WrFixed(nvec.x, 2,6);WrStr("/");WrFixed(nvec.y, 2,1);
                WrStr("/");WrFixed(nvec.z, 2,1); */
               /*WrFixed(dd, 0,8); */
               ...
               if (alticolour) {
                ...
                /* pixel/m */
                  ...
                  /*            pixsizew:=one(pixsize*(PI/180.0*200.0)); */
                ...
                /* size of srtm is limit for aliasing */
                  ...
                  /*
                              IF istree THEN
                                treecol(pos, wx, treehints[odd], sunrotvec,
                pixsizewood, sunlum, wooddens,
                                one(1.0-(snowalt-oldh)/slowaltslant), xsun,
                attw, suncolair, hittree, treec);
                              END;
                  */
                  meadowcol(*pos, slant, oldh, pixsize, xsun, &xsunm, &col);
                /* meadow with snow destruction */
                  rockcol(*pos, slant, oldh, pixsize, xsun, attw, istree,
                &xsunm, &col); /*glance,*/ /* green/rock */
                  citycol(attu, *pos, pixsize, &col, &citylamp); /* city */
                  botaniccol(&boc, suncolair, xsunm, 1.0f, sunlum);
                /* apply sun */
                  snowdense = snowdensity(*pos, slant, oldh, pixsize,
                treehints[odd], attw);
                  snowcol(&snowc, suncolair, snownoise(*pos, xsun, pixsize),
                sunlum);
                ...
                suncolair, &hittree, &hc);
                  }
                  /*              colblend(pixsize*hittree*treerastersize,
                hc, treec, hc); */
                  /* lake */
                ...
                *0.3f*pixsizewater);
                ...
                one((float)(X2C_DIVL(ddm,maxdist)-0.02))+0.01f, sunarea,
                (float)maxdist, 0U, suncol, waterpos, waterh,
                (float)refrac, &hc);
                  /*egde fog */
...
/* make altitude to colour */
{
   ...
   if (atab.len==0UL) {
      ...
      */
      ...
      memset((char *)tlut,(char)0,sizeof(struct _4 [5]));
         ...
         { /* with */
            ...
            if (anonym0->b<=5U) {
               ...
               if (anonym0->b>0U && tlut[anonym0->b-1U].on) {
                  { /* with */
                     struct _4 * anonym1 = &tlut[anonym0->b-1U];
